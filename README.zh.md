...
为 Pi 添加 Codex 上下文窗口、Responses 压缩和图像生成。

本项目是 [awoaCrim/pi-openai-toolkit](https://github.com/awoaCrim/pi-openai-toolkit) 的分支。已移除联网搜索和工具调用审查，保留 Codex Astra 兼容层。

现有 `webSearch` 和 `autoMode` 设置仅为配置兼容而保留，无法启用已移除的功能。本分支不提供审批门禁。
...
默认情况下，符合条件的模型启用远程压缩 v2；远程上下文窗口和生图关闭。使用相关功能仍需后端支持。

## 快速开始：启用远程上下文

如果只需要生图，可跳到[常见用法](#常见用法)。
